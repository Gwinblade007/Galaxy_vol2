...
var player = {
    ...
    //вертикально
    sprite: new Sprite('img/111.png', [0, 0], [39, 39], 16,[0,1],'vertical')
};
...
function handleInput(dt) {
    ...
    if(input.isDown('SPACE') &&
       !isGameOver &&
       Date.now() - lastFire > 700) {
        var x = player.pos[0] + player.sprite.size[0] / 4;
    ...
    for(var i=0; i<enemies.length; i++) {
        ...
        if(boxCollides(pos, size, player.pos, player.sprite.size)) {
            ...
            gameOver();
        		...
        		if(lives>1){player.pos = [canvas.width / 2, 1000];
        		lives--;}
